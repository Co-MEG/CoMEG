...
def unex_patterns_embedding(adjacency, context, context_csc, extents, intents, r=0, y=0, min_support=0, max_support=np.inf, beta=0, 
            degs=[], unexs_g=[], unexs_a=[], unexs=[], names_col=[], comp_gen_graph=None, emb_model=None, names=[]) -> List:
    ...
    for j in np.arange(context.shape[1])[y:]:
        ...
        # Form a new "extent" by selecting all nodes which doc2vec representations are within a given distance from set of attributes representation using the same doc2vec model. Doc2Vec model is pre-trained on the whole node-attribute matrix of the graph.
        valid_nodes = []
        threshold = 0.5
        attributes = np.asarray(list((set(intents[r]).union(set([j])))))
        most_similar_nodes = emb_model.dv.most_similar(emb_model.infer_vector(names_col[attributes], alpha=0.005, epochs=100), topn=100)
        for i, s in most_similar_nodes:
            if s > threshold:
                valid_nodes.append(i)
        ext_j = set(valid_nodes)
        extents[r_new] = list(sorted(set(extents[r]).intersection(ext_j)))
        len_new_extent = len(extents[r_new])
        if len_new_extent >= min_support:
            print(f'Valid nodes: {names[valid_nodes]} (attribute being evaluated: {names_col[attributes]}, {attributes})')
            print(f'Intersection of nodes: {extents[r_new]}')
                ...
                if len_new_extent - len(extents[r]) == 0:
                    ...
                    if is_canno:
                        ...
                        if unex - unexs[ptr] >= 0 or r == 0:
                            ...
                            unex_patterns_embedding(adjacency, context, context_csc, extents, intents, r=r_new, y=j+1, min_support=min_support, 
                                        max_support=max_support, beta=beta, degs=degs, unexs_g=unexs_g, 
                                        unexs_a=unexs_a, unexs=unexs, names_col=names_col, comp_gen_graph=comp_gen_graph, emb_model=emb_model, names=names)
                        else:
...
def run_unex_patterns(adjacency, biadjacency, words, complexity_gen_graphs, order_attributes, s, beta, outfile, names):
    ...
    d2v_model_wikivitals = gensim.models.doc2vec.Doc2Vec.load('notebooks/doc2vec_wikivitals.model') 

    # Algorithm
    with open(f'log_{outfile}_emb_trim', 'w') as f:
        with redirect_stdout(f):
            print('starts profiling...')
            lp = LineProfiler()
            lp_wrapper = lp(unex_patterns_embedding)
            lp_wrapper(adjacency, filt_biadjacency, filt_biadjacency_csc, extents, intents, r=0, y=0, 
                                    ...
                                    comp_gen_graph=complexity_gen_graphs, emb_model=d2v_model_wikivitals, names=names)
            lp.print_stats()

    res = [*zip(extents, intents)]
    
    # Save result
    with open(f"result_{outfile}_emb.bin", "wb") as output:
